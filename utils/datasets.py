...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # 解决中文路径问题
            try:
                img0 = cv2.imdecode(np.fromfile(path, dtype=np.uint8), cv2.IMREAD_COLOR)  # 使用 numpy 处理中文路径
            except:
                img0 = cv2.imread(path)  # 回退到原方法
...
def load_image(self, i):
    ...
    if im is None:  # 如果未缓存到内存
        ...
        if npy and npy.exists():  # 如果.npy文件存在，则加载
            ...
            # 解决中文路径问题
            try:
                im = cv2.imdecode(np.fromfile(path, dtype=np.uint8), cv2.IMREAD_COLOR)  # 使用 numpy 处理中文路径
            except:
                im = cv2.imread(path)  # 回退到原方法
