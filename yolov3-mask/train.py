...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov3-mask.yaml', help='model.yaml path')  # cfg: 网络模型配置文件 包括nc、depth_multiple、width_multiple、anchors、backbone、head等
    parser.add_argument('--data', type=str, default=ROOT / 'data/mask.yaml', help='dataset.yaml path') # data: 实现数据集配置文件 包括path、train、val、test、nc、names等
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch.yaml', help='hyperparameters path') # hyp: 训练时的超参文件
    parser.add_argument('--epochs', type=int, default=200)  # epochs: 训练轮次
